...
/* Read count sectors from drive, starting at lba.  Return a new buffer */
static void *read_sectors(uint64_t lba, unsigned int count)
{
    unsigned char *data;
    off_t pos = 0;
...
static void do_boot(uint64_t lba)
{
    ...
    boot_img_hdr = read_sectors(lba, 1 * SECTORS_PER_PAGE);
    if (!boot_img_hdr) {
    ...
    kernel_data = read_sectors(lba, (kernel_len / SECTOR) + 1);
    if (!kernel_data) {
    ...
    initramfs_data = read_sectors(lba, (initramfs_len / SECTOR) + 1);
    if (!initramfs_data) {
